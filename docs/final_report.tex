...
To make the system closer to a real moderation scenario, a threshold-based manual review analysis was performed using the confidence scores from the realistic CLIP-embedding MLP predictions. Instead of forcing the model to automatically decide every advertisement, predictions with low confidence were routed to manual review. The remaining high-confidence examples were treated as automatic decisions.
...
\midrule
0.50 & 32 & 0.37\% & 0.9067 & 0.6485 \\
0.60 & 194 & 2.24\% & 0.9139 & 0.6617 \\
0.70 & 410 & 4.74\% & 0.9227 & 0.6861 \\
0.80 & 637 & 7.36\% & 0.9328 & 0.7154 \\
0.90 & 961 & 11.11\% & 0.9433 & 0.7424 \\
\bottomrule
\end{tabular}
...
The results show a clear trade-off between automation and reliability. When the confidence threshold is increased, more advertisements are sent to manual review, but the performance on the automatically decided examples improves. For example, using a threshold of 0.70 sends only 4.74\% of the test examples to review, while increasing the accuracy of automatic decisions to 0.9227 and the Macro-F1 score to 0.6861. A stricter threshold of 0.90 sends 11.11\% of the examples to review and increases the automatic-decision Macro-F1 score to 0.7424.
...
The manual review threshold analysis adds an operational perspective. It shows that the model does not need to make a final automatic decision for every advertisement. By routing uncertain cases to manual review, the system can improve the reliability of automatic decisions while keeping the review workload relatively small.
...
\begin{enumerate}
    ...
    \item Further improve the manual review policy by calibrating confidence scores and combining confidence thresholds with unsafe-score thresholds.
    \item Analyze more qualitative examples in order to understand which visual and textual patterns help or hurt the model.
\end{enumerate}

\section{Conclusion}

This project developed a complete pipeline for advertisement moderation classification. The pipeline included dataset mapping, train-validation-test splitting, model training, final evaluation, partial fine-tuning, confidence scoring, manual review threshold analysis, and error analysis.
